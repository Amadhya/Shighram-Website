...
class Checkout extends Component {
  ...
  onPay = () => {
    const {order: {details, user}, paymentPending, paymentSuccess} = this.props;

    let options = {
      "key": "", // Enter the Key ID generated from the Dashboard
      "amount": details.amount, // Amount is in currency subunits. Default currency is INR. Hence, 50000 refers to 50000 paise or INR 500.
      "currency": "INR",
      "name": "Smart Parking",
      "description": "Parking Payement",
      "image": "https://example.com/your_logo",
      "order_id": details.razorpay_order_id,//This is a sample Order ID. Create an Order using Orders API. (https://razorpay.com/docs/payment-gateway/orders/integration/#step-1-create-an-order). Refer the Checkout form table given below
  ...
  renderPaymentDetails = () => {
    const {order: {details}} = this.props;

    return (
      <Fragment>
        <DetailWrapper>
          <Typography variant="body1" gutterBottom>Payment&nbsp;Due:&nbsp;₹&nbsp;{parseFloat(details.amount)/100.00}</Typography>
          <Typography variant="body1" gutterBottom>Location:&nbsp;{details.location}</Typography>
          <Typography variant="body1" gutterBottom>RFID&nbsp;Number:&nbsp;{details.rfid}</Typography>
          <Typography variant="body1">Entry&nbsp;time:&nbsp;{new Date(details.created_on).toString()}</Typography>
        </DetailWrapper>
        <br/>
